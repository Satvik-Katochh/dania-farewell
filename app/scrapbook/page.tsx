...
const Page = ({ page, isLeft }: { page: ScrapbookPage; isLeft: boolean }) => (
  <motion.div
    className={`w-full h-full bg-pink-50 p-2 sm:p-4 ${
      isLeft ? "rounded-l-3xl" : "rounded-r-3xl"
    } shadow-inner flex flex-col items-center justify-center`}
    initial={{ opacity: 0, scale: 0.8 }}
    animate={{ opacity: 1, scale: 1 }}
  ...
  >
    <motion.div
      className="relative w-full h-48 sm:h-4/5 mb-2 sm:mb-4"
      whileHover={{ scale: 1.05 }}
      transition={{ type: "spring", stiffness: 300 }}
    >
      <Image
    ...
    <motion.p
      className="text-gray-700 mb-1 sm:mb-2 font-handwriting text-center text-sm sm:text-xl"
      initial={{ y: 20, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      transition={{ delay: 0.2 }}
    >
    ...
    <motion.span
      className="text-xs sm:text-sm text-gray-500 font-handwriting"
      initial={{ y: 20, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      transition={{ delay: 0.3 }}
    >
...
export default function EnhancedScrapbook() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-200 to-purple-300 p-2 sm:p-8 flex items-center justify-center overflow-hidden">
      <Confetti
        width={width}
        height={height}
        recycle={false}
      ...
      />
      <motion.div
        className="w-full max-w-6xl bg-pink-300 rounded-3xl shadow-2xl overflow-hidden relative p-2 sm:p-8"
        initial={{ opacity: 0, y: 50 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, type: "spring" }}
      >
        <motion.h1
          className="text-2xl sm:text-4xl font-bold text-white text-center mb-4 sm:mb-8 drop-shadow-lg"
          initial={{ y: -50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.5, type: "spring", stiffness: 120 }}
        >
        ...
        <div className="flex justify-center items-center">
          <motion.button
            onClick={() => flipPage("prev")}
            disabled={currentPage === 0}
            className="text-white disabled:text-pink-200 mr-2 sm:mr-4"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            aria-label="Previous page"
          >
          ...
          <div className="w-full max-w-4xl aspect-[3/2] bg-pink-100 rounded-3xl shadow-2xl relative overflow-hidden">
            ...
            <motion.div
              className="absolute inset-y-0 left-1/2 w-0.5 sm:w-1 bg-pink-300 shadow-lg"
              initial={{ height: 0 }}
              animate={{ height: "100%" }}
              transition={{ delay: 0.2, duration: 0.5 }}
            ></motion.div>
          ...
          <motion.button
            onClick={() => flipPage("next")}
            disabled={currentPage === scrapbookPages.length - 1}
            className="text-white disabled:text-pink-200 ml-2 sm:ml-4"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            aria-label="Next page"
          >
